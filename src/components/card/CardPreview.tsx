...
export function CardPreview({ data, showBranding = true }: CardPreviewProps) {
  ...
  const socialLinks = [
    ...
    { id: 'facebook', icon: Facebook, value: data.facebook, url: `https://facebook.com/${data.facebook}` },
    ...
    { id: 'website', icon: Globe, value: data.website, url: data.website?.startsWith('http') ? data.website : `https://${data.website}` },
  ];

  // Limit socials and services for free users
  const activeSocials = isPro 
    ? socialLinks.filter(s => s.value)
    : socialLinks.filter(s => s.value).slice(0, 1);

  const servicesLimit = isPro ? 8 : 3;
  const activeServices = data.services?.filter(s => s.trim() !== '').slice(0, servicesLimit) || [];
      ...
      <div className="relative bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/50 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] overflow-hidden rounded-[2.5rem]">
        ...
        <div className="px-6 pb-10 -mt-14 relative z-10 flex flex-col items-center">
          ...
          <div className="relative mb-6">
            <motion.div 
              ...
              className="w-28 h-28 rounded-[2rem] bg-white dark:bg-slate-900 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] dark:shadow-none p-1.5 relative overflow-hidden"
            >
          ...
          <div className="text-center mb-8 w-full">
            ...
            {data.city && (
              <div className="flex items-center justify-center gap-1.5 mt-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
            ...
            <div className="flex items-center justify-between gap-2.5 pt-2">
              <div className="flex flex-wrap items-center gap-2.5">
                {activeSocials.map((social) => (
                  <motion.a
                    key={social.id}
                    whileHover={{ y: -3, backgroundColor: 'var(--slate-100)' }}
                    whileTap={{ scale: 0.9 }}
                    href={social.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 flex items-center justify-center rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-primary transition-all shadow-sm"
                  >
              ...
              <button
                onClick={handleShare}
                className="w-12 h-12 flex items-center justify-center rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
              >
        ...
        {showBranding && (
          <div className={cn(
            ...
            isPro ? "opacity-0 h-0 p-0 pointer-events-none" : "bg-slate-50/50 dark:bg-slate-950/50 border-slate-100 dark:border-slate-900"
          )}>
            {!isPro && (
